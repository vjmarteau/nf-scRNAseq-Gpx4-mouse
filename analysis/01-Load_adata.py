...
meta = pd.read_csv("../tables/samplesheet_Grabherr.csv", index_col="internal_id")
...
for s in adata.obs["sample"].cat.categories:
    index_w_sel_sample = (
        adata.obs.where(adata.obs["sample"] == s).dropna(how="all").index
    )
    adata.obs.loc[index_w_sel_sample, "sample_counts"] = (
        adata.obs["sample"].value_counts().loc[s]
    )
adata.obs["sample_counts"] = adata.obs["sample_counts"].astype(int)

adata.obs["label"] = (
    adata.obs["label"].astype(str) + "\nn=" + adata.obs["sample_counts"].astype(str)
)
